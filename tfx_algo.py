...
class DNNModel:
    ...
    def build(self):
        ...
        self._model.add(tf.keras.layers.Dense(units=self._init_layer, activation='sigmoid', input_shape=[self._init_layer,]))
        # self._model.add(tf.keras.layers.Dropout(0.8))
        self._model.add(tf.keras.layers.Dense(units=21, activation='sigmoid'))
        # self._model.add(tf.keras.layers.Dense(units=8, activation='relu'))
    ...
    def train(self):
        self._hist_obj = self._model.fit(self._train["data"], self._train['labels'], validation_split=0.20, verbose=1, epochs=200, batch_size=32)#, callbacks=[earlystopping])
